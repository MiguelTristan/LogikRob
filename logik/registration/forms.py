...
class UserCreationFormWhitEmail(UserCreationForm):
    email = forms.EmailField(required=True, help_text="Requerido, maximo 254 caracteres y valido")
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError('El email ya esta registrado, intenta con un email unico')
        return email
